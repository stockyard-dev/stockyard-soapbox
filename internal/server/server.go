package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-soapbox/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-soapbox"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/questions",s.handleList)
    s.mux.HandleFunc("POST /api/questions",s.handleAsk)
    s.mux.HandleFunc("POST /api/questions/{id}/vote",s.handleVote)
    s.mux.HandleFunc("GET /api/questions/{id}/answers",s.handleListAnswers)
    s.mux.HandleFunc("POST /api/questions/{id}/answers",s.handleAnswer)
    s.mux.HandleFunc("POST /api/answers/{id}/accept",s.handleAccept)
