...
<div class="hdr"><h1><span>&#9670;</span> SOAPBOX</h1><button class="btn btn-p" onclick="openForm()">+ Add</button></div>
...
async function load(){var r=await fetch(A+'/questions').then(function(r){return r.json()});items=r.questions||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)||(i.author||'').toLowerCase().includes(q)||(i.tags||'').toLowerCase().includes(q)});
...
if(i.body)h+='<div class="item-sub">'+esc(i.body)+'</div>';
...
if(i.answer_count)h+='<span>'+esc(i.answer_count)+'</span>';
if(i.accepted)h+='<span>'+esc(i.accepted)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
if(i.body)h+='<div class="item-notes">'+esc((i.body||'').substring(0,200))+'</div>';
...
function formHTML(item){var i=item||{title:"",body:"",author:"",tags:"",votes:"",answer_count:"",accepted:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SOAPBOX</h2>';
...
h+='<div class="fr"><label>Author</label><input id="f-author" value="'+esc(i.author)+'"></div>';
...
h+='<div class="fr"><label>Answer Count</label><input id="f-answer-count" value="'+esc(i.answer_count)+'"></div>';
h+='<div class="fr"><label>Accepted</label><input id="f-accepted" value="'+esc(i.accepted)+'"></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),body:document.getElementById('f-body').value.trim(),author:document.getElementById('f-author').value.trim(),tags:document.getElementById('f-tags').value.trim(),votes:document.getElementById('f-votes').value.trim(),answer_count:document.getElementById('f-answer-count').value.trim(),accepted:document.getElementById('f-accepted').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/questions/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/questions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
